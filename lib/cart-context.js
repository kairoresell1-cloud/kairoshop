...
export function CartProvider({ children }) {
  ...
  // Niente spedizione se nel carrello ci sono SOLO prodotti digitali.
  // Basta un solo prodotto fisico per far scattare la spedizione fissa.
  const hasPhysicalItem = items.some((i) => !i.isDigital);
  const isAllDigital = items.length > 0 && !hasPhysicalItem;
  const shippingFee = isAllDigital ? 0 : SHIPPING_FEE;

  // La spedizione (se applicabile) può comunque essere annullata da una
  // promozione attiva (free shipping), verificata server-side al checkout.
